...
const SelectProduct = () => {
  ...
  const { data } = useGetShopifyDataQuery({ id: "9565173186875" });
    ...
    <div className="sp_bg poppinsFont py-5">
      <div className="max-w-5xl mx-auto  px-3  ">
        <div className="grid sm:grid-cols-2 items-center sm:gap-6" id="form">
          ...
          <div className="">
            <div className="fontPoppins font-extrabold sm:text-[2.2vw] text-[8vw]">
              Ashwarin Plus Powder
            </div>
            <div className="md:text-xl text-md fontPoppins text-gray-500"> Ayurvedic Weight & Muscle Gain Powder</div>
        ...
        <div className="grid md:grid-cols-2 items-center gap-4 selectedItem ">
          {allVariants?.map((variant, key) => {
            return (
              <label
                ...
                value={variant.id}
                htmlFor={variant.id}
                className={`relative border border-gray-500 border-solid rounded-3xl cursor-pointer`}
              >
                ...
                <div
                  className={`fontPoppins text-center rounded-t-3xl py-2 ${selectProducts?.id === variant?.id ? "bg-[#05ca4c]" : "bg-gray-300"
                    }`}
                >
                ...
                <div
                  className={`rounded-b-3xl  p-2 sm:p-4  ${selectProducts?.id === variant?.id ? "activeStatus2" : ""
                    }`}
                >
